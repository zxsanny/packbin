# Test Specification — Python package
...
**Test procedure**:
1. Build the archive the tag job would upload to PyPI `packbin`
...
| Step | Action | Expected Result |
|------|--------|-----------------|
| 1 | push a version tag | this package is on PyPI `packbin` |
| 2 | install with `pip install packbin` | the package imports |
...
**Summary**: The archive on PyPI `packbin` declares the MIT license.
