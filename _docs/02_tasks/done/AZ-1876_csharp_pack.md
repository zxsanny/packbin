...
the caller's server needs the same bytes as the other five languages. A tagged serializer would change the size.
...
- The package is what NuGet `Packbin` publishes
...
- The package identity NuGet `Packbin`, license MIT
