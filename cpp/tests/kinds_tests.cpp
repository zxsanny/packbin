...
void new_field_kinds() {
  auto empty = packbin::scheme(1, {packbin::flags("f", {packbin::group("mark", {})})});
  packbin::Values mark;
  ...
  auto one = packbin::scheme(
      1, {packbin::flags("f", {packbin::u8("a"), packbin::u8("b"), packbin::u8("c"), packbin::u8("d"),
                               packbin::u8("e"), packbin::u16("b5")})});
  packbin::Values a;
  a.emplace("a", packbin::Value{std::uint8_t{1}});
  expect(packbin::pack(one, a).size() == 3, "one field bit length 3");
  packbin::Values wide;
  ...
  auto two = packbin::scheme(
      1, {packbin::flags("f", {packbin::group("session", {packbin::u16("login"), packbin::u32("ts")})})});
  ...
  auto zero = packbin::scheme(1, {packbin::flags("f", {packbin::group("g", {packbin::u8("b")})})});
  ...
  auto short_got = packbin::unpack(two, std::vector<std::uint8_t>{0x01, 0x01, 0x07});
  ...
  auto sized_layout = packbin::scheme(1, {packbin::u16("n"), packbin::sized("payload", "n")});
  ...
  expect(packbin::to_hex(packbin::pack(sized_layout, sized_v)) == "010300756176", "sized 3");
  auto sized_got = packbin::unpack(sized_layout, packbin::pack(sized_layout, sized_v));
  ...
  expect(packbin::to_hex(packbin::pack(sized_layout, empty_v)) == "010000", "sized 0");
  auto empty_got = packbin::unpack(sized_layout, packbin::pack(sized_layout, empty_v));
  expect(empty_got.ok &&
             std::get<packbin::Value::Bytes>(empty_got.value.at("payload").data).empty(),
         "sized 0 unpack");
  auto sized_short = packbin::unpack(sized_layout, parse_hex("01030075"));
  ...
  auto kinds = packbin::scheme(1, {packbin::u2({"a", "b", "c", "d"})});
  ...
  expect(packbin::to_hex(packbin::pack(packbin::scheme(1, {packbin::u2({"a"})}), one_v)) == "0101",
         "u2 one");

  auto bits_layout = packbin::scheme(1, {packbin::u8("n"), packbin::bits("segs", "n")});
  ...
  auto eight_bytes = packbin::pack(bits_layout, eight_v);
  ...
  auto nine_bytes = packbin::pack(bits_layout, nine_v);
  expect(nine_bytes.size() == 4 && nine_bytes[0] == 0x01 && nine_bytes[2] == 0xff &&
             (nine_bytes[3] & 0xfe) == 0,
         "bits 9 unused 0");
  auto bits_short = packbin::unpack(bits_layout, std::vector<std::uint8_t>{1, 9, 0x01});
...
void utf8_string() {
  auto layout = packbin::scheme(1, {packbin::utf8("name")});
  packbin::Values vals;
  vals.emplace("name", packbin::Value{"zxsanny"});
  auto raw = packbin::pack(layout, vals);
  ...
  auto empty = packbin::pack(layout, empty_vals);
  expect(packbin::to_hex(empty) == "010000", "utf8 empty");
  auto empty_got = packbin::unpack(layout, empty);
  ...
  bool failed = false;
  try {
    packbin::pack(layout, long_vals);
  } catch (std::runtime_error const&) {
    failed = true;
  }
  expect(failed, "utf8 too long");

  auto short_got = packbin::unpack(layout, parse_hex("0107007a78"));
...
void counted_list() {
  auto two = packbin::scheme(1, {packbin::list("xs", packbin::u16("n"))});
  ...
  auto be_one = packbin::scheme(1, {packbin::list("xs", packbin::be(packbin::u16("n")))});
  packbin::Values be_vals;
  be_vals.emplace("xs", packbin::Value{be_items});
  expect(packbin::to_hex(packbin::pack(be_one, be_vals)) == "0101000001", "list be");

  auto one = std::make_shared<packbin::ValueList>();
  one->items.push_back(packbin::Value{std::uint8_t{1}});
  auto followed = packbin::scheme(1, {packbin::list("xs", packbin::u8("n")), packbin::u8("y")});
  ...
  expect(packbin::to_hex(packbin::pack(two, empty_vals)) == "010000", "list empty");
...
void dictionary() {
  ...
  auto empty = packbin::scheme(
      1, {packbin::utf8("s"), packbin::list("xs", packbin::u8("n")), packbin::dict("m", packbin::utf8("v"))});
  packbin::Values empty_vals;
  ...
  expect(packbin::to_hex(packbin::pack(empty, empty_vals)) == "01000000000000", "dict empty");

  auto dup = packbin::scheme(1, {packbin::dict("access", packbin::utf8("v"))});
  auto bad = packbin::unpack(dup, parse_hex("010200010061010078010061010079"));
